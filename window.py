...
class Window:
    ...
    def drawWindow(self):
        ...
        self.equalButton = Button(self.rootWindow, text="=", width=self.buttonWidth, height=self.buttonHeight * 2 + 1, command=lambda: self.count())
    ...
    def addText(self, str):
        """Method called by buttons to add char to string displayed in screen text field
    ...
    def count(self):
        equation = self.screenWindow.get('1.0', 'end-1c')
        rpn = RPN()
        result = rpn.count(equation)
        if not result:
            result = "error"
            
        self.screenWindow.delete('1.0', 'end')
        self.screenWindow.insert('1.0', result)
        self.historyWindow.insert('end', str("{} = {}\n").format(equation, result))
